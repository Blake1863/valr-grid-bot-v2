...
export class RiskManager {
  ...
  /** Validate qty would not exceed maxNetPosition */
  checkMaxPosition(currentQty: Decimal, addQty: Decimal): void {
    const maxNet = new Decimal(this.config.maxNetPosition);
    const newTotal = currentQty.plus(addQty);
    if (newTotal.gt(maxNet)) {
      throw new Error(
        `Max net position exceeded: current=${currentQty}, adding=${addQty}, max=${maxNet}`
      );
    }
  }
