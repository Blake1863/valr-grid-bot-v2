...
export async function reconcile(
  ...
  // For neutral mode, we need both buy AND sell sides populated
  ...
  if (config.mode === 'neutral') {
    ...
    needsGridPlacement = activeBuys < config.levels || activeSells < config.levels;
    if (needsGridPlacement) {
      log.info({ activeBuys, activeSells, required: config.levels }, 'Neutral mode: grid incomplete');
    }
  } else {
