...
**Open position with take profit and stop loss (RECOMMENDED: use trading-stop after entry)**
```
# Step 1: Open the position
POST /v5/order/create
{"category":"linear","symbol":"BTCUSDT","side":"Buy","orderType":"Market","qty":"0.01","positionIdx":0}

# Step 2: Attach TP/SL using trading-stop (immediately after fill)
POST /v5/position/trading-stop
{"category":"linear","symbol":"BTCUSDT","positionIdx":0,"takeProfit":"90000","stopLoss":"78000","tpslMode":"Full"}
```

> **Why trading-stop?** The `/v5/position/trading-stop` endpoint is the reliable way to attach TP/SL to an existing position. TP/SL are stored on the position itself — when the position closes (manually, by TP, or by SL), they auto-clear. No orphaned orders.

**Open position with take profit and stop loss (alternative: attach to entry order)**
```
POST /v5/order/create
...
> ⚠️ **Caveat**: TP/SL attached to entry order may not set correctly if the order fills before TP/SL are processed. Prefer `trading-stop` after entry for reliability.

**Add/modify TP/SL on existing position**
```
POST /v5/position/trading-stop
{"category":"linear","symbol":"BTCUSDT","positionIdx":0,"takeProfit":"92000","stopLoss":"79000","tpslMode":"Full"}
```

> Use this to add TP/SL to a position that was opened without them, or to modify existing levels. Pass `"0"` to cancel (e.g., `"stopLoss":"0"` cancels SL).
...
### Recommended Pattern: Use trading-stop Endpoint
...
# Attach TP/SL using trading-stop (reliable, works on existing positions)
POST /v5/position/trading-stop
{"category":"linear","symbol":"BTCUSDT","positionIdx":0,"takeProfit":"90000","stopLoss":"78000","tpslMode":"Full"}
```

**Key points:**
- `trading-stop` endpoint is the reliable way to set TP/SL on any position
- TP/SL are stored on the position itself, not as separate orders
- When position closes (manually, TP, or SL), TP/SL auto-clear — no orphaned orders
- Cancel TP or SL by passing `"0"` (e.g., `"stopLoss":"0"` cancels SL)
- Can modify TP/SL anytime by calling `trading-stop` again with new prices
